...
most money. Write a query that returns one city that has the highest sum of invoice totals. 
...
SELECT customer.customer_id, first_name, last_name, sum(total) AS amount_spent
FROM customer
...
SELECT DISTINCT email, first_name, last_name, g.name AS g_name
FROM customer AS c
...
Track, Customer, Album, and Artist tables. Note, this one is tricky because the Total spent in the Invoice table might 
not be on a single product, so you need to use the InvoiceLine table to find out how many of each product was purchased,
